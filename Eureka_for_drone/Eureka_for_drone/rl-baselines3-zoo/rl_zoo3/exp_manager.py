...
class LoggerCallback(BaseCallback):
    ...
    def _on_training_end(self):
        # Save collected data to a .npz file for analysis
        np.savez(os.path.join(self.log_dir, "training_metrics.npz"),
                 rewards=self.episode_rewards,
                 actions=self.episode_actions,
                 values=self.episode_values,
                 log_probs=self.episode_log_probs,
                 lengths=self.episode_lengths)

    def _on_rollout_end(self):
        # Record metrics at the end of each episode
        self.episode_rewards.append(self.locals['rewards'])
        self.episode_actions.append(self.locals['actions'])
        self.episode_values.append(self.locals['values'].cpu().numpy())
        self.episode_log_probs.append(self.locals['log_probs'].cpu().numpy())
        self.episode_lengths.append(len(self.locals['dones']))
...
class ExperimentManager:
    ...
    def learn(self, model: BaseAlgorithm) -> None:
        ...
        if len(self.callbacks) > 0:
            self.callbacks.append(LoggerCallback(self.save_path))
